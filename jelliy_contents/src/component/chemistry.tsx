  ...
  return (
    <div className="component_container">
      ...
      <section className='first_view chemistry' ref={firstViewRef} style={sectionStyle(0)}>
        <h1 className='chemistry_h1'>化学</h1>
        ...
        <div className='merit_and_demerit_block scroller_decoration'>
          <article className={`merit ${animateMerit ? 'animate' : ''}`}>
            <h2>特長</h2>
            <h3>物質の変化の理解</h3><p>化学は物質の組成や変化を理解するための科学であり、この理解は様々な分野での応用につながります。例えば、医薬品の開発や材料工学において重要な役割を果たしています。</p>
            <h3>実験と理論の統合</h3><p>化学は実験と理論を組み合わせることが一般的であり、理論的な予測を実際の実験結果と照らし合わせることで、新たな知識の発見が可能です。</p>
            <h3>持続可能な技術の発展</h3><p>化学は環境への影響を考慮した持続可能な技術の発展にも寄与しています。再生可能エネルギーや環境に優しい製品の開発などがこれに該当します。</p>
            <h3>医学や農業への応用</h3><p>化学は医学や農業においても欠かせない要素であり、新しい薬剤や肥料の開発などにおいて進歩を促進しています。</p>
          </article>
          <article className={`demerit ${animateDemerit ? 'animate' : ''}`}>
            <h2>難点</h2>
            <h3>環境への影響</h3><p>化学プロセスや化学物質の製造はしばしば環境に対する影響を持ちます。有害な副産物や廃棄物の生成が問題視され、環境への負荷が懸念されています。</p>
            <h3>安全性の問題</h3><p>化学実験や製造過程において、取り扱う物質が有毒である場合があります。これにより、作業者や周辺の人々の安全性に関する懸念が生じることがあります。</p>
